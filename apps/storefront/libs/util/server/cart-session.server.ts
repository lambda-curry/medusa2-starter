...
export const getCartSession = async (headers: Headers): Promise<Partial<CartSessionData>> => {
  ...
  const region = !regionId ? (await client.regions.list({}))['regions'][0] : null;
...
export const createCartSession = async (headers: Headers, createData: CartSessionData) => {
  ...
  if (createData.cartId !== cartId) await setCookie(headers, cartIdCookie, createData.cartId);
  if (createData.regionId !== regionId) await setCookie(headers, regionIdCookie, createData.regionId);
  if (createData.currencyCode !== currencyCode) await setCookie(headers, currencyCodeCookie, createData.currencyCode);
