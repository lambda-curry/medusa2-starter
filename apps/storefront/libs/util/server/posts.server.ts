...
export type MappedDataSections =
  | ProductGridPostSection
  | ProductCarouselPostSection
  | BlogListPostSection
...
export const getBlogPostListData = async (
  ...
  const { limit, offset, searchParams } = withPaginationParams({
    request,
    prefix,
  })
    ...
    if (include_collection_tabs || include_category_tabs) {
      ...
      if (include_collection_tabs) {
        collection_tabs = collection_tab_id?.length
          ? collection_tabs
              .filter((c) => {
                const filterOption = filterOptions.collections.find(
                  (co) => co.id === c.id,
                )

                return (
                  collection_tab_id?.includes(c.id) &&
                  !!filterOption?.product_count
                )
              })
              .sort(
                (a, b) =>
                  collection_tab_id?.indexOf(a.id) -
                  collection_tab_id?.indexOf(b.id),
              )
          : collection_tabs
      } else {
        collection_tabs = []
      ...
      if (include_category_tabs) {
        ...
        category_tabs = category_tab_id?.length
          ? category_tabs
              .filter((c) => {
                const filterOption = filterOptions.categories.find(
                  (co) => co.id === c.id,
                )
              ...
              .sort(
                (a, b) =>
                  category_tab_id?.indexOf(a.id) -
                  category_tab_id?.indexOf(b.id),
              )
          : category_tabs
      } else {
        category_tabs = []
...
const PostSectionDataMap: Record<
  string,
  (section: any, request: Request) => Promise<any>
> = {
