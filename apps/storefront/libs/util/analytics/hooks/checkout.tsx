...
export const useCheckoutAnalytics = () => {
  ...
  const checkoutFetcher = fetchers.find(f => f.formAction?.includes('/api/checkout'));
  const sendSignUpEvent = useSendEvent('sign_up');
  const sendAddShippingInfoEvent = useSendEvent('add_shipping_info');

  useEffect(() => {
    if (checkoutFetcher?.data && checkoutFetcher?.formData && checkoutFetcher?.state === 'loading') {
