...
export const getVariantBySelectedOptions = (
  ...
  variants.find((v) =>
    v.options?.every((o) => options[o.option_id!] === o.value),
  )
...
export const selectDiscountFromVariant: (
  variant?: StoreProductVariant,
) => VariantDiscount | undefined = (variant) => {
  if (!variant) return
...
const generateOptionCombinations = (
  ...
  return (
    first.values?.reduce(
      (acc: string[][], productOptionValue: StoreProductOptionValue) => {
        const value = productOptionValue.value
        const newCombinations: string[][] = subCombinations.map((sub) => [
          value,
          ...sub,
        ])
      ...
      ?.values?.filter((optionValue) => {
        ...
        return product.variants?.some((variant) => {
          const variantOptionIds =
            variant.options?.map((option) => option.option_id) || []
...
export const getOptionValuesWithDiscountLabels = (
  ...
  return optionValues.map((optionValue) => {
    const currentOptionWithSelectOptions = selectedOptions?.map(
...
export const getMergedProductMeta = mergeMeta(
  getParentMeta,
  getCommonMeta,
  getProductMeta,
  getPostMeta,
