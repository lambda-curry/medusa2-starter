...
import type { ContextValue } from "@libs/utils-to-merge/types"
...
export interface SetPayloadWithTarget<T extends SettableTargets>
  extends SetPayload<T> {
  target: T
}
...
export interface TogglePayloadWithTarget<T extends ToggleableTargets>
  extends TogglePayload<T> {
  target: T
}
...
export type StorefrontContextValue = ContextValue<
  StorefrontState,
  StorefrontAction
>

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const StorefrontContext = createContext<StorefrontContextValue>(
  null as any,
)
...
const actionHandlers = {
  set: (
    ...
    { target, state: newTargetState }: SetPayloadWithTarget<SettableTargets>,
  ) => ({
    ...state,
  ...
  toggle: (
    ...
    }: TogglePayloadWithTarget<ToggleableTargets>,
  ) => ({
    ...state,
    [target]: merge({}, state[target], newTargetState, {
...
export const StorefrontProvider: FC<StorefrontProviderProps> = ({
  data,
  ...props
}) => {
  ...
  return (
    <StorefrontContext.Provider
      value={{ state: merge({}, state, data), dispatch }}
      {...props}
    />
  )
}
