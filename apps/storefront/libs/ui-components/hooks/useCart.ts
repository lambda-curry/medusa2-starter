...
export const useCart = () => {
  ...
  const removingLineItemFetcher =
    removingLineItemFetchers[removingLineItemFetchers.length - 1]

  if (
    cartFetcher &&
    cartFetcher.formMethod === "POST" &&
    ["loading", "submitting"].includes(cartFetcher.state)
  )
    isAddingItem = true

  if (
    removingLineItemFetcher &&
    ["loading", "submitting"].includes(removingLineItemFetcher.state)
  )
