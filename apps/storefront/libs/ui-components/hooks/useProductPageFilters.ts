...
import { ProductsIndexRouteLoader } from '../../../app/routes/products._index';
import { FilterOptions, nestCategories } from '@marketplace/util/product-filters';

export const parseProductPageFilterSearchParams = (filterSearchParams: string) =>
  qs.parse(decodeURIComponent(filterSearchParams), { parseArrays: true, comma: true }) as unknown as Record<
    string,
    string[] | string
  >;
...
export const useProductPageFilters: ({
...
}) => ProductPageFiltersHook = ({ filterOptions = {}, allFilterOptions = {} }) => {
  const productRouteData = useRouteLoaderData<ProductsIndexRouteLoader>('routes/products._index');
  ...
  const formattedFilters: FilterOptions = useMemo(() => {
    ...
    if (clonedFilters.categories) clonedFilters.categories = nestCategories(clonedFilters.categories);
  ...
  const formattedAllFilters: FilterOptions = useMemo(() => {
    ...
    if (clonedAllFilters.categories) clonedAllFilters.categories = nestCategories(clonedAllFilters.categories);
  ...
  const selectedCollections =
    typeof filterQuery?.collections === 'string' ? [filterQuery.collections] : filterQuery.collections || [];
  ...
  const clearSelectedCollections = () => setSearchParams({ filters: updateFilterQuery('collections', []) });

  const selectedCategories =
    typeof filterQuery?.categories === 'string' ? [filterQuery.categories] : filterQuery.categories || [];
  const setSelectedCategories = (categories: string[]) => {
    const newQuery = updateFilterQuery('categories', categories);
    setSearchParams({ filters: newQuery });
  };
  const clearSelectedCategories = () => setSearchParams({ filters: updateFilterQuery('categories', []) });

  const selectedTags = typeof filterQuery.tags === 'string' ? [filterQuery.tags] : filterQuery.tags || [];
  const setSelectedTags = (tags: string[]) => {
    const newQuery = updateFilterQuery('tags', tags);
    setSearchParams({ filters: newQuery });
  };
  const clearSelectedTags = () => setSearchParams({ filters: updateFilterQuery('tags', []) });

  const selectedOrder = typeof filterQuery?.order === 'string' ? filterQuery.order : 'popularity';
