...
import {
  ...
  ToggleableTargets,
  TogglePayload,
} from '../providers/storefront-provider'
...
const actions = ({ dispatch }: StorefrontContextValue) => ({
  toggleCartDrawer: (
    payload?: TogglePayload<ToggleableTargets.cart> | boolean,
  ) => dispatch(toggleActionDispatch(ToggleableTargets.cart, payload)),
  toggleLoginModal: (
    payload?: TogglePayload<ToggleableTargets.login> | boolean,
  ) => dispatch(toggleActionDispatch(ToggleableTargets.login, payload)),
  toggleSearchDrawer: (payload?: boolean) =>
    dispatch(toggleActionDispatch(ToggleableTargets.search, payload)),
})
...
export const useStorefront = () => {
  ...
  if (!context)
    throw new Error(
      'useStorefront must be used within a StorefrontContext.Provider',
    )
